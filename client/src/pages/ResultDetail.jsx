...
export default function ResultDetail() {
  ...
  const downloadPdf = async () => {
    try {
      ...
      pdf.text(
        `Test Date: ${new Date(result.createdAt).toLocaleString()}`,
        90,
        userTop + 70
      );
      ...
      const metrics = [
        ...
        {
          label: "Avg Time",
          value: `${(result.avgTimeSec || 0).toFixed(2)}s`,
          color: orange,
        },
      ];
      let x = 20;
      ...
      pdf.setTextColor(...slate);
      pdf.setFontSize(12);
      pdf.text(`Total Questions: ${result.totalQuestions}`, 40, y);
      y += 18;
      pdf.text(`Correct: ${result.correct}`, 40, y);
      y += 18;
      pdf.text(`Wrong: ${result.wrong}`, 40, y);
      y += 18;
      pdf.text(
        `Average Time per Question: ${(result.avgTimeSec || 0).toFixed(2)}s`,
        40,
        y
      );
      y += 28;
      ...
      const stamp = new Date(result?.createdAt || Date.now())
        .toISOString()
        .slice(0, 19)
        .replace(/[:T]/g, "-");
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {user?.gender ? (
                <div className="text-blue-700 text-xs capitalize">
                  {user.gender}
                </div>
              ) : null}
